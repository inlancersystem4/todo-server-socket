...
io.on("connection", (socket) => {
  ...
  socket.on("editing", (data, userId) => {
    console.log("Editing.....", userId, socket.id);
    io.emit("userBlock", { data, editing_user: userId });
  });

  socket.on("editingComplete", (data) => {
    io.emit("editingComplete", { data });
  });
  ...
  socket.on("user_status", (data) => {
    ...
    console.log(
      `User status received: ${contact_id} is ${status} in room ${room_id}`
    );
